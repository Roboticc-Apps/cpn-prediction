...
On a Riemannian manifold, the Raychaudhuri equation \cite{raychaudhuri1955} governs the evolution of the expansion scalar $\theta$ of a geodesic congruence:
\begin{equation}
    \frac{d\theta}{dt} = -\frac{1}{d}\theta^2 - \sigma_{ab}\sigma^{ab} + \omega_{ab}\omega^{ab} - R_{ij}u^i u^j
    \label{eq:raychaudhuri}
\end{equation}
where $d = 2n$ is the real dimension, $\sigma_{ab}$ is the shear tensor, $\omega_{ab}$ is the vorticity tensor, and $u^i$ is the unit tangent vector to the geodesic.
...
\begin{proof}
For an initially parallel congruence, $\theta_0 = 0$ and $\sigma = \omega = 0$ at $t = 0$. From (\ref{eq:raychaudhuri}) and (\ref{eq:ricci}):
\[
\left.\frac{d\theta}{dt}\right|_{t=0} = -\frac{1}{2n} \cdot 0 - 0 + 0 - R_{ij}u^i u^j = -2(n+1)
\]
...
\begin{proof}
Consider the Raychaudhuri equation (\ref{eq:raychaudhuri}) with $\sigma = \omega = 0$ (irrotational, shear-free congruence). Set $\lambda = 2(n+1)$ for the Ricci term. Then:
\begin{equation}
    \frac{d\theta}{dt} = -\frac{1}{2n}\theta^2 - \lambda
    \label{eq:ray_simple}
\end{equation}

\textit{Case 1:} $\theta_0 \leq 0$. The right-hand side of (\ref{eq:ray_simple}) is strictly negative (both terms $\leq 0$, with $-\lambda < 0$ always). So $\theta$ is strictly decreasing. Since $d\theta/dt \leq -\lambda < 0$, we have $\theta(t) \leq \theta_0 - \lambda t \to -\infty$.

\textit{Case 2:} $\theta_0 > 0$. The equation $d\theta/dt = -\theta^2/(2n) - \lambda$ has the property that $d\theta/dt < 0$ whenever $\theta^2 > 2n\lambda$, i.e., $|\theta| > \sqrt{2n\lambda}$. For $\theta_0 > \sqrt{2n\lambda}$, $d\theta/dt < 0$ immediately.

For any $\theta_0 > 0$, consider the comparison equation $d\theta/dt = -\lambda$ (dropping the $-\theta^2/(2n)$ term when $\theta > 0$). This gives $\theta(t) \leq \theta_0 - \lambda t$, so $\theta$ reaches zero by time $t_0 \leq \theta_0/\lambda$. After that, Case 1 applies.

In both cases, $\theta \to -\infty$ in finite time. The caustic time is bounded by:
\begin{equation}
    t_{\text{caustic}} \leq \frac{\theta_0}{\lambda} + \frac{\pi}{\sqrt{\lambda}} = \frac{\theta_0}{2(n+1)} + \frac{\pi}{\sqrt{2(n+1)}}
\end{equation}
\end{proof}
...
\bottomrule
\end{tabular}
\caption{Time to contraction ($\theta = 0$) for initially expanding congruences on $\mathbb{CP}^7$ ($\lambda = 16$). The quadratic term $-\theta^2/(2n)$ accelerates contraction beyond the linear bound, making the actual contraction time significantly shorter than the bound for large $\theta_0$.}
\label{tab:expansion}
\end{table}
