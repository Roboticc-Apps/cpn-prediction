...
50 & $\mathbb{CP}^{2^{50}-1}$ & $2 \times 10^{15}$ & $2 \times 10^{15}$ \\
\bottomrule
\end{tabular}
...
\begin{conjecture}[Euclidean error scaling]
\label{thm:scaling}
The systematic error introduced by Euclidean averaging on the state space of a $q$-qubit system scales approximately as $2^q$ with qubit count. Simulation and hardware data are consistent with:
\begin{equation}
...
The measured gain ratio approaches $2\times$ at higher qubit counts (the $4q/3q$ ratio of $1.98\times$ is nearly exact), consistent with the doubling of the focusing rate. The initial super-doubling ($4.83\times$ at $2q/1q$) occurs because the 1-qubit manifold ($\mathbb{CP}^1$, the Bloch sphere) is low-dimensional, and the sub-doubling at $5q/4q$ reflects saturation effects as Euclidean fidelity approaches the random-guessing floor.
...
This geometric error is an additional factor---alongside decoherence, gate errors, and crosstalk---that contributes to the difficulty of scaling quantum advantage. Unlike those engineering challenges, the scaling ceiling is a mathematical obstruction inherent to the state space geometry.
...
The Bures correction is complementary to, not redundant with, maximum likelihood estimation. In \cite{muir2026}, combining MLE reconstruction with Bures averaging (MLE+Bures) outperformed or matched both MLE alone and Bures alone in all tested configurations.
...
Australian Patent Application 2026901876 (2026).
